...
// export default async function handlePlayableCard(io, tableId, userId) {

//     console.log("=== HANDLE PLAYABLE CARD CALLED ===", new Date().toISOString());
...
//     const isRoundEnd = pli.size === playersId.length;

//     console.log("is round end", isRoundEnd)
//     if (isRoundEnd) {
...
//     console.log('end handle playable card')
...
export async function areHandsEmpties(playersId){
    let handEmpty = true 
    for (const playerId of playersId){
        const player = await usersCollection.findOne({ _id: new ObjectId(playerId)});
